<x-admin-layout>
    <x-slot name="header">Tambah Reservasi</x-slot>

    <x-admin-form-card title="Reservasi Baru" description="Buat reservasi secara manual untuk pelanggan." :back="route('admin.reservations.index')">
        <form method="POST" action="{{ route('admin.reservations.store') }}" class="max-w-2xl space-y-5">
            @csrf
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="number" min="1" max="20" id="guests" name="guests" value="{{ old('guests') }}"
            ...
            <div>
                ...
                <select id="table_id" name="table_id"
                    ...
                    <option value="" disabled {{ old('table_id') ? '' : 'selected' }}>Pilih meja…</option>
                    @foreach($tables as $table)
                        <option value="{{ $table->id }}" @selected(old('table_id') == $table->id)>{{ $table->name }} — {{ $table->capacity }} tamu</option>
                    @endforeach
                </select>
                @if($tables->isEmpty())
                    <p class="mt-1 text-sm text-amber-600">Tidak ada meja tersedia saat ini.</p>
                @endif
